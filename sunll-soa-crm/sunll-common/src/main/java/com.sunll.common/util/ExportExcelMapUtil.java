 ...
 */
public class ExportExcelMapUtil<T> {
    ...
    public static void exportExcel(HttpServletResponse response, String fileName, String[] headers, List contactsList) throws Exception {
        ...
        if (contactsList != null && contactsList.size() > 0) {
            ...
            exportExcel(fileName, headers, out2, DateUtil.SQL_TIME_FMT, count);
        }
        try {
            if (out2 != null) {
                out2.close();
    ...
    public static void exportExcel(String title, String[] params,
        ...
        if (params != null && params.length > 0) {
            ...
            while (it.hasNext()) {
                ...
                Map<String, String> t = (Map<String, String>) it.next();
                //改造 拿到id集合循环map里根据key值查找value值
                for (int i = 0; i < cloums.length; i = i + 1) {
                    HSSFCell cell = row.createCell(i);
                    cell.setCellStyle(style2);
                    try {
                        Class<?> tCls = t.getClass();
                        Object value = t.get(cloums[i]);
                        ...
                        short ts = isHasCount(cindex, (short) i);
                            /*if (ts != -1) {
                                count[ts] = count[ts]
...
//			}
     ...
     */
    @SuppressWarnings("deprecation")
    public static InputStream exportExcel(String title, String[] params,
