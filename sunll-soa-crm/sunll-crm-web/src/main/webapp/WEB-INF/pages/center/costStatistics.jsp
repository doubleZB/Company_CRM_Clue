    ...
    <div class="admin-content-body">
        <div class="am-tabs"  data-am-tabs="{noSwipe: 1}">
            <ul class="am-tabs-nav am-nav am-nav-tabs">
                <li id="manage_tab1" class="am-active"><a href="#tab1">语音费用统计</a></li>
            </ul>
                ...
                <div class="am-tab-panel am-active" id="tab1">
                    ...
                    <table class="am-table am-table-striped">
                        ...
                        <tr v-for="item in json">
                            ...
                            <td>{{item.userName}}</td>
                            <td>0</td>
                            <td>{{item.callTotalTime}}</td>
                            <td>{{item.seatCostTotal}}</td>
                            <td>{{item.callCostTotal}}</td>
                            <td>{{item.total}}</td>
...
<script>
    ...
    function reloadPage(pageNumber){
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/cost/getData?pageNumber="+pageNumber+"&pageSize="+pageSize,
            type:'POST',
            async:false,
            ...
            success: function(data){
                ...
                //总计最后一行
                var userNameSum = "全员";
                var callTotalTimeSum = 0;
                var callCostTotalSum = 0;
                var totalSum = 0;
                //总计最后一行
                for (var i = 0; i < data.list.length; i++) {
                    ...
                    //toFixed(2)
                    data.list[i].seatCostTotal = data.list[i].seatCostTotal.toFixed(2);
                    data.list[i].callCostTotal = data.list[i].callCostTotal.toFixed(2);
                    data.list[i].total = data.list[i].total.toFixed(2);
                };

                $.ajax({
                    url: "${pageContext.request.contextPath}/cost/allData",
                    type:'POST',
                    async:false,
                    dataType:"json",
                    error:function(){
                    ...
                    success: function(data) {
                        for (var i = 0; i < data.list.length; i++) {
                            ...
                            callTotalTimeSum = callTotalTimeSum + data.list[i].callTotalTime;
                            callCostTotalSum = callCostTotalSum + data.list[i].callCostTotal;
                            totalSum = totalSum + data.list[i].total;
                            //总计最后一行
                        };
                    }
                });
                ...
                //toFixed(2)
                callCostTotalSum = callCostTotalSum.toFixed(2);
                ...
                "<td>"+userNameSum+"</td>"+
                "<td>0</td>"+
                "<td>"+callTotalTimeSum+"</td>"+
                "<td>0.00</td>"+
                "<td>"+callCostTotalSum+"</td>"+
                "<td>"+totalSum+"</td></tr>";
