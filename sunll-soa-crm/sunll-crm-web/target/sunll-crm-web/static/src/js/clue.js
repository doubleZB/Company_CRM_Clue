...
var vm = new Vue({
    ...
    methods: {
        ...
        saveUploadFile: function(){
            ...
            var extName = file.substring(file.lastIndexOf(".")+1,file.length+1);
            console.log(extName == "xls" ? '1' :'0')

            if(extName == "xls" || extName== "xlsx"){
                var formData = new FormData($("#uploadForm")[0]);
                $.ajax({
                    url:"/clue/importClues/"+sessionStorage.getItem("USER_TOKEN")+"/"+sessionStorage.getItem("CRM_COMPANY_TOKEN")+"/"+vm.checkBoxId+"/"+vm.businessSceneFont,
                    type: 'POST',
                    data: formData,
                    async: false,
                    cache: false,
                    ...
                    success:function(data){
                        if(!data.success){
                            if(!data.header){
                                ...
                                vm.errorInfo=data.url;
                                $("#errorId").show();
                                $("#errorMessage").hide();
                                $("#errorMessageExcel").attr(data.message);
                            }
                        }else {
                            $("#errorId").hide();
                            if (data.data == "1") {
        ...
        setAllSelected: function (isAllSelected) {
            ...
            if (isAllSelected) {
                ...
                this.currentSelected = this.rows.map(function(row){ return row.filedId});
                //$(".iptcheckbox").prop("checked",true);
            } else {
                //this.have = true;
                //$(".smsShow").attr("data-target","");
                this.currentSelected = [];
                //$(".iptcheckbox").prop("checked",false);
...
//                var index = this.showFiled.indexOf(filter);
        ...
        changeItem: function (event, item, index) {
          ...
          /*  const id = item.filedId;
            ...
            var flog1=true;
            for(var i=0;i<$(".iptcheckbox").length;i++){
                if(!$(".iptcheckbox").eq(i).is(":checked")){
                    flog1=false;
                }
            }
            ...
            console.log($('.iptcheckbox:checked').length)
            if($('.iptcheckbox:checked').length <= 0){
               //vm.have=true;
            }
            if($('.iptcheckbox:checked').length > 0){
                //vm.have = false;
    ...
    computed: {
        ...
        isAllSelected: function () {
            return this.currentSelected.length === this.rows.length && this.rows.length > 0;
        }
        ,
        // 用于展示的数据，即应用了过滤条件的数据
        filtedData: function () {
            return this.rows.map(function(row) {
                ...
                for (let i = 0; i < vm.showFiled.length; i++) {
                    let filter = vm.showFiled[i];
...
function listCule(pageNumber) {
    ...
    if( this.tableList.length>0) {
        this.tableList.map(function(parent) {
            let obj = {};
            obj['filedId'] = parent.id;
            parent.data.map(function(child) {
...
function  selectCheckBox(){
    vm.checkBoxId=$('input:radio:checked').val();
}
function uploadErrorFile(){
  location.href="/clue/uploadErrorFile?fileName="+vm.errorInfo
/*    var data = {
        "version": "V1.0",
        "data": {
            "fileURL": vm.errorInfo
        }
    };
    $.ajax({
        url: "/clue/uploadErrorFile",
        type: 'POST',
        async: false,
        data: JSON.stringify(data),
        dataType: "json",
