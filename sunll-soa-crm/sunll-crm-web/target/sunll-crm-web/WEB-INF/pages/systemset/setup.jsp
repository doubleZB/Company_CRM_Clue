...
<div id="setUp">
    <nav class="navbar-default navbar-static-side" role="navigation">
        ...
        <%--    <ul class="nav metismenu" id="side-menu">
                &lt;%&ndash;<li v-for="item in navList" class="{{ active }}" v-if="item.menuStatus==2" ><!-- menuStatus：1前台2下拉3后台 menuLevel：1一级菜单2二级菜单 -->&ndash;%&gt;
                <li v-for="item in navList" class="{{ active }}" v-if="item.menuStatus==2" class="scale">
            ...
            <ul class="nav metismenu" id="side-menu">
                <li v-for="item in navList" v-if="item.menuStatus==2" class="menuList">
        ...
        methods: {
         ...
         /*   iframeHref : function(url, e){
                if (url) {
                    ...
                    if($(e.target).parents(".scale").find("ul").attr("status")==1){
                        $(e.target).parents(".scale").find("ul").attr("status",2);
                        $(e.target).parents(".scale").find("ul").slideDown();
                    }else{
                        $(e.target).parents(".scale").find("ul").attr("status",1);
                        $(e.target).parents(".scale").find("ul").slideUp();
                    }
                    console.log(1111)
//                    console.log($(e.target).parents(".menuList").find("ul").style.display=="none")
//                    if($(e.target).parents(".menuList").find("ul").style.display=="none"){
